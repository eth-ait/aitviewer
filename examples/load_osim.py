...
def display_model_in_viewer(osim: Optional[str] = None, 
    ...
    if args.mot is None:
        osim_seq = OSIMSequence.a_pose(args.osim, name='OpenSim template', 
        ...
        osim_seq = OSIMSequence.from_files(args.osim, args.mot, 
                                        show_joint_angles=args.joints, 
                                        color_skeleton_per_part=args.color_parts, 
                                        color_markers_per_part=args.color_markers,
                                        fps_out=args.fps)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Load an OpenSim model and a motion file and display it in the viewer.')
    ...
    parser.add_argument('--mocap', type=str, help='If a Mocap file is specified, display the markers mocap with their labels. For now, only the .c3d format is supported.')
